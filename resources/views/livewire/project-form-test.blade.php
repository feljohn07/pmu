    ...
    <form wire:submit.prevent="save" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" wire:model.defer="projectName" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            ...
            <div>
                ...
                <input type="text" wire:model.defer="project_category" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            ...
            <div>
                ...
                <input disabled type="text" wire:model.defer="appropriation" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            ...
            <div>
                ...
                <input type="text" wire:model.defer="source_of_funds" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            ...
            <div>
                ...
                <input type="text" wire:model.defer="duration" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            ...
            <div>
                ...
                <input type="date" wire:model.defer="start_date" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                ...
                <div class="mt-6 border p-4 rounded bg-gray-50 shadow">
                    ...
                    {{-- Use wire:key for efficient DOM updates in loops --}}
                     @foreach ($this->{$groupKey} as $index => $cost)
                        <div wire:key="{{ $groupKey }}-{{ $index }}" class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-6 gap-3 items-end mb-3 p-2 border-b border-gray-200 last:border-b-0">
                           {{-- Description --}}
                            <div class="sm:col-span-3 md:col-span-2">
                                <label for="{{ $groupKey }}_{{ $index }}_desc" class="text-sm font-medium text-gray-600">Description</label>
                                <input type="text" id="{{ $groupKey }}_{{ $index }}_desc" wire:model.defer="{{ $groupKey }}.{{ $index }}.cost_description"
                                    class="w-full mt-1 border-gray-300 rounded-md shadow-sm text-sm p-1.5">
                                @error("{$groupKey}.{$index}.cost_description") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                            {{-- Percentage --}}
                            {{-- <div>
                                <label for="{{ $groupKey }}_{{ $index }}_perc" class="text-sm font-medium text-gray-600">Percentage (%)</label>
                                <input type="number" step="0.01" id="{{ $groupKey }}_{{ $index }}_perc" wire:model.defer="{{ $groupKey }}.{{ $index }}.percentage"
                                    class="w-full mt-1 border-gray-300 rounded-md shadow-sm text-sm p-1.5">
                                 @error("{$groupKey}.{$index}.percentage") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div> --}}
                            {{-- Amount --}}
                           <div>
                                <label for="{{ $groupKey }}_{{ $index }}_amount" class="text-sm font-medium text-gray-600">Amount</label>
                                <input type="number" step="0.01" id="{{ $groupKey }}_{{ $index }}_amount" wire:model.defer="{{ $groupKey }}.{{ $index }}.amount"
                                    class="w-full mt-1 border-gray-300 rounded-md shadow-sm text-sm p-1.5">
                                 @error("{$groupKey}.{$index}.amount") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                           </div>
                           {{-- Action Buttons --}}
                    ...
                    <button type="button" wire:click="addRowAt('{{ $groupKey }}', {{ count($this->{$groupKey}) - 1 }}, 'below')"
